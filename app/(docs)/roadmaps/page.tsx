    ...
    <div className="flex flex-col justify-center items-center flex-grow px-4 sm:px-6 lg:px-8 mt-20 sm:mt-12 lg:mt-0">
          ...
          {roadmapData.map((item, idx) => (
            <Card key={sectionKeys[idx]} className="p-4 sm:p-6 h-full">
              <CardHeader className="pb-4">
                ...
                <CardDescription className="text-sm sm:text-base mt-2">{item.description}</CardDescription>
              </CardHeader>
              <CardContent className="pt-0">
                <ul className="ml-2 sm:ml-4 space-y-2 sm:space-y-3">
                  {items[idx]?.map((_, stepIdx) => (
                    <li key={stepIdx} className="flex items-start gap-2 text-sm sm:text-base">
                      <span className="w-1.5 h-1.5 rounded-full bg-primary mt-2 flex-shrink-0" />
                      <span className="leading-relaxed">{t(`sections.${sectionKeys[idx]}.steps.${stepIdx}`)}</span>
                    </li>
                  ))}
                </ul>
              </CardContent>
      ...
      <section id="paths" className="w-full max-w-7xl mt-8 sm:mt-12 lg:mt-16">
        <div className="text-center mb-6 sm:mb-8 lg:mb-12">
          <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold">Choose Your Path</h3>
        </div>
      ...
      <section id="faq" className="w-full max-w-4xl mt-8 sm:mt-12 lg:mt-16">
        <div className="text-center mb-6 sm:mb-8">
          <h3 className="text-xl sm:text-2xl lg:text-3xl font-bold">Frequently Asked Questions</h3>
        </div>
        <div className="mx-auto">
          <Accordion type="single" collapsible className="w-full space-y-2">
            {faqKeys.map((key) => (
              <AccordionItem key={key} value={key} className="border rounded-lg px-4 sm:px-6">
      ...
      {/* Resources Section */}
      <section id="resources" className="w-full max-w-7xl mt-8 sm:mt-12 lg:mt-16 mb-8 sm:mb-12">
        <div className="container mx-auto text-center">
          <h2 className="text-2xl sm:text-3xl lg:text-4xl xl:text-5xl font-bold tracking-tight">Developer Resources</h2>
          <p className="max-w-3xl mx-auto mt-3 sm:mt-4 lg:mt-6 text-sm sm:text-base lg:text-lg xl:text-xl text-muted-foreground leading-relaxed px-4">
            Browse a list of free courses, guides, practice platforms, open-source projects, and tools.
          </p>
        </div>
        <div className="grid gap-4 sm:gap-6 lg:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 mt-6 sm:mt-8 lg:mt-12">
          {resources.map((res) => (
            <ResourceCard key={res.id} title={res.title} Icon={res.Icon} links={res.links} />
          ))}
        </div>
      </section>
    </div>
